...
wb_enqueue_common (wb_inode_t *wb_inode, call_stub_t *stub, int tempted)
{
	...
	req->ordering.tempted = tempted;
		...
		/* req->write_size can change as we collapse
		   ...
		   最初的size，这个size就是应用发送的size，由于后来的write_size可能会由于合并等原因发生
		   变化，所以不能决定unwind window的大小，unwind window的大小由orig_size决定。
		*/
	...
	case GF_FOP_WRITE:
		LOCK (&wb_inode->lock);
		{
			if (wb_inode->size < stub->args.offset) {
				/* 如果写操作的offset大于当前文件尾了，则将offset更新为文件尾部的offset
				然后增加写操作的size，增量就是offset与文件尾部的差值，后续的操作中会将
				文件尾部到offset之间填充为0，也就是形成了空洞文件。
				*/
				...
				/*写操作的offset小于或等于文件尾，意味着覆盖写，或者追加写。offset和size
				不会变化。
				*/
				req->ordering.off = stub->args.offset;
				req->ordering.size = req->write_size;
			}
			/*如果写入改变了文件尾部，即文件大小增加了，则在这里更新文件尾部
			*/
...
__wb_preprocess_winds (wb_inode_t *wb_inode)
{
	...
	*/
	...
	conf = wb_inode->this->private;
		...
		if (req->stub->args.offset != offset_expected) {/* 两个异步操作的区域不连续，不能合并 */
			holder->ordering.go = 1;
			holder = req;
			continue;
		}

		if (!is_same_lkowner (&req->lk_owner, &holder->lk_owner)) {/* 锁拥有者不一致，不能合并 */
			holder->ordering.go = 1;
			holder = req;
			continue;
		}
		...
		if (space_left < req->write_size) {/* 用于合并的剩余空间不足，不能合并 */
			holder->ordering.go = 1;
			holder = req;
			continue;
		}
