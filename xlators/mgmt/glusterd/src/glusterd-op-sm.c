...
/*根据txn_id获取opinfo，opinfo存放在priv->glusterd_txn_opinfo字典中。
*/
int32_t
...
/*将opinfo赋值到this->private的glusterd_txn_opinfo字典中，以txn_id为key。如果其中还没有该key的
*	value，则分配相应的空间后在赋值。
*/
int32_t
...
glusterd_op_ac_send_brick_op (glusterd_op_sm_event_t *event, void *ctx)
{
		...
		/* 如果已经没有了pending brick和pending node，则清空pending_bricks，进入下一个事件(状态)
		*/
...
/*注入事件，为event结构分配内存，并初始化，然后添加到一个全局的事件队列中。
ctx----记录命令行信息的字典
event_type----初始状态时的事件类型
txn_id----传输id，标记本次操作，作为key，可以在private->glusterd_txn_opinfo字典中找到相应的opinfo
*/
int
...
glusterd_op_sm ()
{
        ...
        while (!cds_list_empty (&gd_op_sm_queue)) {
                ...
                cds_list_for_each_entry_safe (event, tmp, &gd_op_sm_queue,
                                              list) {
				/*遍历gd_op_sm_queue队列，取出一个event。
				*/
                        cds_list_del_init (&event->list);//从队列中删除该事件，因为下面会从处理它，所以不需要继续保留在队列中。
