...
{
    xlator_t *        xl;
    int32_t           healers;//正在执行的heal任务个数
    int32_t           heal_waiters;//正在等待执行的heal任务个数
    int32_t           nodes;
    ...
    uint32_t          background_heals;//允许同时执行的heal任务数量
    uint32_t          heal_wait_qlen;//允许等待的heal任务数量
    struct list_head  pending_fops;
    struct list_head  heal_waiting;//等待执行的heal任务
    struct list_head  healing;//正在执行的heal任务
